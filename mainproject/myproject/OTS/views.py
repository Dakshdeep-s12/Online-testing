...
def candidateHome(request):
    if 'name' not in request.sesssion.keys():
...
def showTestResult(request):
    if 'name'  not in request.session.keys():
        res=HttpResponseRedirect("login")
    result=Result.objects.filter(resultid=Result.objects.latest('resultid').resultid,username_id=request.session)
...
def testResultHistory(request):
    ...
    context={'candidate':candidate,'result':results}
    res=render(request,'candidate.history.html',context)
    return res
